...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT song, COUNT(DISTINCT albums.asin)
    FROM albums
    JOIN tracks
      ON albums.asin = tracks.album
    GROUP BY song
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT style, AVG(price/num_tracks) AS price_per_track
    FROM styles
    JOIN (
      SELECT albums.*, COUNT(*) AS num_tracks
          ...
          GROUP BY asin) AS album_track
      ON styles.album = album_track.asin
    GROUP BY style
    ORDER BY AVG(price/num_tracks) DESC
    LIMIT 5
  SQL
end
